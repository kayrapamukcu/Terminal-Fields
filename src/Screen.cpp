...
void Screen::renderBarStats(int x, int y, int health, int maxHealth, int mana, int maxMana, bool renderXPBar) {
    ...
    sf::Color manaColor = sf::Color::Blue;
    if (maxMana <= 0) {
        manaColor = sf::Color(64, 64, 64);
    }
    ...
    for (int i = 1; i < 20; i++) {
        if (health >= (static_cast<float>(maxHealth) / 19.0f) * i) {
            ...
            Screen::updateTerminalColor(x + i - 1, y + 1, 1, 1, sf::Color(96 + static_cast<int>(159 * std::fmod(health, hpPerBlock) / hpPerBlock), 0, 0));
            break;
        }
    }
    ...
    if (maxMana > 0) {
        ...
        for (int i = 1; i < 20; i++) {
            if (mana >= (static_cast<float>(maxMana) / 19.0f) * i) {
                ...
                Screen::updateTerminalColor(x + i - 1, y + 4, 1, 1, sf::Color(0, 0, 96 + static_cast<int>(159 * std::fmod(mana, mpPerBlock) / mpPerBlock)));
                break;
            }
            Screen::updateTerminalColor(x, y + 4, 1, manaToBeFilled, sf::Color::Blue);
        }
    } else {
        Screen::updateTerminal(x, y + 3, "MP\x3", false, sf::Color(64, 64, 64));
        Screen::updateTerminal(x, y + 4, barSize, false, sf::Color(64, 64, 64));
    }
    ...
    if (renderXPBar) {
        ...
        for (int i = 1; i < 20; i++) {
            if (Player::xp >= (Player::xpToLevelUp / 19.0f) * i) {
                xpToBeFilled++;
            } else {
                Screen::updateTerminalColor(x + i - 1, y + 7, 1, 1, sf::Color(32 + static_cast<int>(54 * std::fmod(Player::xp, xpPerBlock) / xpPerBlock), 96 + static_cast<int>(159 * std::fmod(Player::xp, xpPerBlock) / xpPerBlock), 0));
                break;
            }
        }
    ...
    statBar[ind] = '\x3';
    Screen::updateTerminal(x + (19 - ind), y + 5, &statBar[0], false, manaColor);
    if (renderXPBar) {
        ...
        // Screen::updateTerminal(x, y+8, &("LVL " + std::to_string(Player::level) + "\x3")[0], false, sf::Color(0, 255, 0));
    }
}
...
void Screen::transition(int direction, float transitionTime, bool clearOrDraw) {  // direction 0 = bottom to top, 1 = top to bottom, 2 = right to left, 3 = left to right, 4 = circling
